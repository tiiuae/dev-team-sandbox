...
# Get integer (ASCII code) from a byte
function Ord {
    printf "%d" "\"$1"
}

# Get byte from an integer (ASCII code)
function Chr {
...
# If read byte is zero, the corresponding variables are unset
# Convert bytes to their ASCII values or zero if unset
if [ -z "$B1" ]; then
    B1=0
else
    B1="$(Ord "$B1")"
...
# Convert the four bytes to a integer length
# Using multiply instead of bit shifing as bit shifts broke syntax highlighting
